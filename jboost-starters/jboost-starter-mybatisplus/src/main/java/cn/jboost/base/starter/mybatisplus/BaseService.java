...
public abstract class BaseService<T, D> {
    ...
    public D findById(Serializable id) {
        return converter.toDTO(mapper.selectById(id));
    }
    ...
    public List<D> findByIds(Collection<? extends Serializable> ids) {
        return (List<D>) converter.toDTO(mapper.selectBatchIds(ids));
    }
     ...
     * @return
     */
    public <Q> List<D> findByCriteria(Q queryCriteria) {
        return (List<D>) converter.toDTO(mapper.selectList(MyBatisPlusQueryHelper.buildQuery(entityType, queryCriteria)));
    }
     ...
     * @return
     */
    public IPage<D> findByWrapper(Wrapper<T> queryWrapper, Page page) {
        return converterPage(mapper.selectPage(page, queryWrapper));
    }
     ...
     * @return
     */
    public IPage<T> findByExample(T example, Page page) {
     ...
     * @return
     */
    public IPage<Map<String, Object>> findMapsByWrapper(Wrapper<T> queryWrapper, Page page) {
    ...
    public boolean saveBatch(Collection<D> dtos, int batchSize) {
        List<T> entities = (List<T>) converter.toEntity(dtos);
     ...
     * @return
     */
    protected IPage<D> converterPage(IPage<T> result) {
        IPage<D> resultDTO = new Page<>();
        resultDTO.setRecords((List<D>) converter.toDTO(result.getRecords()));
